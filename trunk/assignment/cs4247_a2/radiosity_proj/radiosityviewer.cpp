	...
	glBegin( GL_QUADS );
		for ( int q = 0; q < m->numQuads; q++ )
		{
			...
			for ( int i = 0; i < 4; i++ )
			{
				...
				//rgb[0] = log( pow( quad->rgb[i][0] / maxColor, 0.2f ) + 1.0f ) / log2;
				//rgb[1] = log( pow( quad->rgb[i][1] / maxColor, 0.2f ) + 1.0f ) / log2;
				//rgb[2] = log( pow( quad->rgb[i][2] / maxColor, 0.2f ) + 1.0f ) / log2;
